...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        curr_s_level = self.s_levels[level]
        self.a_levels.append(set())

        act_legit = True
        for act in self.all_actions:
            pga = PgNode_a(act)
            for pre_st in pga.prenodes:
                if pre_st not in curr_s_level:
                    act_legit = False
                    break
            if act_legit:
                self.a_levels[level].add(pga)
                for st in pga.prenodes:
                    pga.parents.add(st)
                    st.children.add(pga)
            act_legit = True
    ...
    def add_literal_level(self, level):
        ...
        # curr_s_level = self.s_levels[level-1]
        curr_a_level = self.a_levels[level-1]
        self.s_levels.append(set())
        for act in curr_a_level:
            for act_n in act.effnodes:
                # new_s_level = act_n
                self.s_levels[level].add(act_n)
                act.children.add(act_n)
                act_n.parents.add(act)

        # new_s_level = self.s_levels[level]
        # for act in curr_a_level:
        #     for st in new_s_level:
        #         if st.literal in act.effnodes:
        #             act.children.add(st)
        #             st.parent.add(act)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        c = False
        for a1_eff in node_a1.action.effect_add:
            if a1_eff in node_a2.action.effect_rem:
                c = True
                break
        return c
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        c1 = False
        for a1_eff in node_a1.action.effect_add:
            if a1_eff in node_a2.action.precond_neg:
                c1 = True
                break
        c2 = False
        for a2_eff in node_a2.action.effect_add:
            if a2_eff in node_a1.action.precond_neg:
                c2 = True
                break
        c3 = False
        for a1_eff in node_a1.action.effect_rem:
            if a1_eff in node_a2.action.precond_pos:
                c3 = True
                break
        c4 = False
        for a2_eff in node_a2.action.effect_rem:
            if a2_eff in node_a1.action.precond_pos:
                c4 = True
                break
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #         break
        # c3 = False
        # for a1_pre in node_a1.action.precond_neg:
        #     if a1_pre in node_a2.action.precond_pos:
        #         c3 = True
        #         break
        # c4 = False
        # for a2_pre in node_a2.action.precond_neg:
        #     if a2_pre in node_a1.action.precond_pos:
        #         c4 = True
        #         break
        # return c1 #or c2 or c3 or c4
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            for idx, st_n in enumerate(s_levels):
                if PgNode_s(goal,True) in st_n:
                    level_sum = level_sum + idx
                    break
        return level_sum
