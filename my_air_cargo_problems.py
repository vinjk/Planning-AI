...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
            ...
            for apt in self.airports:
                for pl in self.planes:
                    for crg in self.cargos:
                        precond_pos = [expr("At({},{})".format(crg,apt)), expr("At({},{})".format(pl, apt))]
                        precond_neg = []
                        effect_add = [expr("In({},{})".format(crg,pl))]
                        effect_rem = [expr("At({},{})".format(crg, apt))]
                        load_act = Action(expr("Load({},{},{})".format(crg,pl,apt)),
                                    [precond_pos, precond_neg],
                                    [effect_add, effect_rem])
                        loads.append(load_act)
        ...
        def unload_actions():
            ...
            for apt in self.airports:
                for pl in self.planes:
                    for crg in self.cargos:
                        precond_pos = [expr("At({},{})".format(pl,apt)), expr("In({},{})".format(crg, pl))]
                        precond_neg = []
                        effect_add = [expr("At({},{})".format(crg,apt))]
                        #effect_rem = [expr("At({},{})".format(pl, apt))]
                        effect_rem = [expr("In({},{})".format(crg, pl))]
                        unload_act = Action(expr("Unload({},{},{})".format(crg,pl,apt)),
                                    [precond_pos, precond_neg],
                                    [effect_add, effect_rem])
    ...
    def actions(self, state: str) -> list:
        ...
        for act in self.actions_list:
            act_legit = True
            for prec_p in act.precond_pos:
                if prec_p not in kb.clauses:
                    act_legit = False
                    break
            if act_legit:
                for prec_n in act.precond_neg:
                    if prec_n in kb.clauses:
                        act_legit = False
                        break
            if act_legit:
